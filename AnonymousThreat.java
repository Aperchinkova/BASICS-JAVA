    ...
    public static void main(String[] args) {
        ...
        while(!command.equals("3:1"))
        {
            if(command.contains("merge"))
            {
                ...
                boolean isValidIndexes=endIndex>=0 && startIndex<= texts.size()-1 && endIndex>startIndex;
                if(isValidIndexes)
                {
                    String resultMerge="";
            ...
            else if(command.contains("divide"))
            {
                ...
                texts.remove(index);//{ghi, jkl}
                int partSize=elementForDivide.length()/parts;//6/3=2->2 per 3 parts
